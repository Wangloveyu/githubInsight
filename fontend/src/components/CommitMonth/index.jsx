  ...
  useEffect(() => {
    ...
    if (chartRef.current && details && details.length !== 0) {
      ...
      details.forEach(item => {
        ...
        Object.entries(item?.commit_frequency?.freq?.Month).forEach(it => {
          mySet.add(it[0])
        })
      ...
      details.forEach(item => {
        const temp = []
        xData.forEach(it => {
          if (item?.commit_frequency?.freq?.Month[it]) {
            temp.push(item.commit_frequency.freq.Month[it])
          } else {
            temp.push(0)
          }
        })
