  ...
  useEffect(() => {
    ...
    if (chartRef.current && details && details.length !== 0) {
      ...
      details.forEach(item => {
        console.log(item)
        if (item?.issue_frequency?.freq?.Month) {
          Object.entries(item?.issue_frequency?.freq?.Month).forEach(it => {
            mySet.add(it[0])
          })
        }
      ...
      details.forEach(item => {
        const temp = []
        xData.forEach(it => {
          if (item?.issue_frequency?.freq?.Month[it]) {
            temp.push(item.issue_frequency.freq.Month[it])
          } else {
            temp.push(0)
          }
        })
